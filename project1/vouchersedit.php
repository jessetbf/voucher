...
// Create page object
$vouchers_edit = new cvouchers_edit();
$Page =& $vouchers_edit;

// Page init
$vouchers_edit->Page_Init();

// Page main
$vouchers_edit->Page_Main();
?>
...
// Create page object
var vouchers_edit = new ew_Page("vouchers_edit");

// page properties
vouchers_edit.PageID = "edit"; // page ID
vouchers_edit.FormID = "fvouchersedit"; // form ID
var EW_PAGE_ID = vouchers_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
vouchers_edit.ValidateForm = function(fobj) {
...
vouchers_edit.Form_CustomValidate =  
...
vouchers_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
vouchers_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $vouchers->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $vouchers->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $vouchers_edit->ShowPageHeader(); ?>
<?php
$vouchers_edit->ShowMessage();
?>
<form name="fvouchersedit" id="fvouchersedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return vouchers_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="vouchers">
...
<?php if ($vouchers->id->Visible) { // id ?>
	<tr id="r_id"<?php echo $vouchers->RowAttributes() ?>>
		...
		<td<?php echo $vouchers->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $vouchers->id->ViewAttributes() ?>><?php echo $vouchers->id->EditValue ?></div>
<input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($vouchers->id->CurrentValue) ?>">
</span><?php echo $vouchers->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
class cvouchers_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$vouchers->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$vouchers->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$vouchers->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$vouchers->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($vouchers->id->CurrentValue == "")
			$this->Page_Terminate("voucherslist.php"); // Invalid key, return to list
		switch ($vouchers->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$vouchers->SendEmail = TRUE; // Send email on update success
				if ($this->EditRow()) { // Update record based on key
					$this->setSuccessMessage($Language->Phrase("UpdateSuccess")); // Update success
					$sReturnUrl = $vouchers->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "vouchersview.php")
						$sReturnUrl = $vouchers->ViewUrl(); // View paging, return to View page directly
		...
		// Render the record
		$vouchers->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$vouchers->ResetAttrs();
		$this->RenderRow();
	}
